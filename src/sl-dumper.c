...
static LibraryType quick_scan_elf(const char *lib_path);

static void add_symbol(uint64_t offset, const char *class_name, const char *method_name);
...
static bool elf_find_sections(const uint8_t *mem, size_t file_size, ElfSections *out) {
    ...
    for (uint32_t i = 0; i < shnum; i++) {
        uint32_t sh_type = is_64
            ? ((Elf64_Shdr *)(mem + shoff + (uint64_t)i * sizeof(Elf64_Shdr)))->sh_type
            : ((Elf32_Shdr *)(mem + shoff + (uint64_t)i * sizeof(Elf32_Shdr)))->sh_type;

        if (sh_type == SHT_DYNSYM) {
            out->symtab = mem + (is_64
                ? ((Elf64_Shdr *)(mem + shoff + (uint64_t)i * sizeof(Elf64_Shdr)))->sh_offset
                : ((Elf32_Shdr *)(mem + shoff + (uint64_t)i * sizeof(Elf32_Shdr)))->sh_offset);
            out->sym_size = is_64
                ? ((Elf64_Shdr *)(mem + shoff + (uint64_t)i * sizeof(Elf64_Shdr)))->sh_size
                : ((Elf32_Shdr *)(mem + shoff + (uint64_t)i * sizeof(Elf32_Shdr)))->sh_size;
            out->sym_ent = is_64
                ? ((Elf64_Shdr *)(mem + shoff + (uint64_t)i * sizeof(Elf64_Shdr)))->sh_entsize
                : ((Elf32_Shdr *)(mem + shoff + (uint64_t)i * sizeof(Elf32_Shdr)))->sh_entsize;

            uint32_t link = is_64
                ? ((Elf64_Shdr *)(mem + shoff + (uint64_t)i * sizeof(Elf64_Shdr)))->sh_link
                : ((Elf32_Shdr *)(mem + shoff + (uint64_t)i * sizeof(Elf32_Shdr)))->sh_link;
            ...
            out->strtab = (const char *)(mem + (is_64
                ? ((Elf64_Shdr *)(mem + shoff + (uint64_t)link * sizeof(Elf64_Shdr)))->sh_offset
                : ((Elf32_Shdr *)(mem + shoff + (uint64_t)link * sizeof(Elf32_Shdr)))->sh_offset));

            // H4: Capture strtab_size for bounds checking
            out->strtab_size = is_64
                ? ((Elf64_Shdr *)(mem + shoff + (uint64_t)link * sizeof(Elf64_Shdr)))->sh_size
                : ((Elf32_Shdr *)(mem + shoff + (uint64_t)link * sizeof(Elf32_Shdr)))->sh_size;
...
static void parse_and_store_demangled(uint64_t offset, const char *demangled) {
    ...
    if (method_start) {
        size_t class_len = (size_t)(method_start - 2 - demangled);
        char class_name[256];
...
static void process_elf(const char *lib_path) {
    ...
    uint8_t *mem = (uint8_t *)mmap(NULL, (size_t)st.st_size, PROT_READ, MAP_PRIVATE, fd, 0);
    close(fd);
    ...
    if (!elf_find_sections(mem, (size_t)st.st_size, &sections)) {
    ...
    // Iterate symbols
    int count = (int)(sections.sym_size / sections.sym_ent);
    for (int i = 0; i < count; i++) {
        ...
        if (sections.is_64) {
            const Elf64_Sym *sym = (const Elf64_Sym *)(sections.symtab + (uint64_t)i * sections.sym_ent);
            st_value = sym->st_value;
            st_name = sym->st_name;
            st_info = sym->st_info;
        } else {
            const Elf32_Sym *sym = (const Elf32_Sym *)(sections.symtab + (uint64_t)i * sections.sym_ent);
            st_value = sym->st_value;
            st_name = sym->st_name;
            st_info = sym->st_info;
        }
...
static LibraryType quick_scan_elf(const char *lib_path) {
    ...
    int scan_count = (int)(sections.sym_size / sections.sym_ent);
...
int main() {
        ...
        while ((dir = readdir(d)) != NULL) {
            if (strstr(dir->d_name, ".so") != NULL) {
                if (file_count >= file_capacity) {
                    ...
                    char **tmp_files = realloc(so_files, (size_t)file_capacity * sizeof(char *));
                    if (!tmp_files) {
                        fprintf(stderr, "Error: Out of memory\n");
                    ...
                    so_files = tmp_files;
                    LibraryType *tmp_types = realloc(lib_types, (size_t)file_capacity * sizeof(LibraryType));
                    if (!tmp_types) {
                        fprintf(stderr, "Error: Out of memory\n");
                        for (int i = 0; i < file_count; i++) free(so_files[i]);
                        free(so_files);
    ...
    double elapsed = (double)(end_time.tv_sec - start_time.tv_sec) + (double)(end_time.tv_nsec - start_time.tv_nsec) / 1e9;
